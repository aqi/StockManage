...
//@user_id,@department_id,@staffinfo_position,@staffinfo_name,@staffinfo_cell,@staffinfo_sex,)";
        ...
        public DataTable Select(Staff staff, int start, int max)
        {
            ...
            if (staff.Role_id == 2)
            {
                commandText.Append(" , ('C' + right('000000' + convert(varchar(8), staffs.staffinfo_id), 7)) staffinfo_bh," + 
                    "isnull(purchase.sum, 0) sum from t_staff staffs left join (select staffinfo_id, " +
...
//                    "where 1=1 "); 
            ...
            {
                commandText.Append(" ,('X' + right('000000' + convert(varchar(8), staffs.staffinfo_id), 7)) staffinfo_bh," + 
                    "isnull(sale.sum, 0) sum from t_staff staffs left join (select staffinfo_id, " +
            ...
            {
                commandText.Append(" ,('K' + right('000000' + convert(varchar(8), staffs.staffinfo_id), 7)) staffinfo_bh," +　
                    "isnull(stock.sum, 0) sum from t_staff staffs left join (select " +
