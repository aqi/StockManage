...
namespace Web0204.BM.BLL
{
    ...
    /// </summary>
    public class ContractShopProvider : ActionBase
    {
        #region --- Insert方法 ---
        ...
        public DataTable Select(Shop shop, int start, int max)
        {
            ...
            commandText.Append("SELECT * FROM t_shop shops,t_good goods,t_contract contracts WHERE shops.good_id=goods.good_id AND shops.contrac_id=contracts.contrac_id ");
            IList parameters = new ArrayList();
            ...
            if (shop.Contrac_id != 0)
            {
                commandText.Append(" AND shops.contrac_id=@contrac_id ");
                DataParameter parmContract = new DataParameter();
                parmContract.ParameterName = "@contrac_id";
                parmContract.DbType = DbType.Int32;
                parmContract.Value = shop.Contrac_id;
        ...
        {
            string commandText = "SELECT * FROM t_shop shops,t_good goods,t_contract contracts WHERE shops.good_id=goods.good_id AND shops.contrac_id=contracts.contrac_id ";
        ...
        {
            string commandText = "SELECT Count(shop_id) FROM t_shop WHERE contrac_id=@contrac_id";
